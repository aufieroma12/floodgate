...
DATA_DIR = Path(__file__).parents[1] / "ishigami" / "data" / "analysis_10k"
...
### Figure 1 ###
fig, ax = plt.subplots(2, d, figsize=(15, 20), gridspec_kw={'height_ratios': [3, 1]})
...
mse_vals = [
    analytical_mse(1 + float(n), 7 + 2 * float(n), 0.1 - 0.5 * float(n)) for n in noise_vals
]
...
for i in range(d):
    ax[0,i].set(xscale='log', title=f"X_i", ylim=(-.01,1))
...
ax[0,1].legend(loc='lower center', bbox_to_anchor=(0.5, -.6), ncol=2, fancybox=True)
