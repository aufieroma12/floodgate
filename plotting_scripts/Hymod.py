...
DATA_DIR = Path(__file__).parents[1] / "Hymod" / "data" / "analysis"
...
surrogate_10k_gt = np.load(DATA_DIR / "ground_truth_surrogate_10000.npy")
surrogate_100k_gt = np.load(DATA_DIR / "ground_truth_surrogate_100000.npy")
...
for fp in os.listdir(DATA_DIR / "floodgate" / "100000"):
    floodgate.append(np.load(DATA_DIR / "floodgate" / "100000" / fp))
...
# Non-surrogate SPF
for fp in os.listdir(DATA_DIR / "spf"):
    spf.append(np.load(DATA_DIR / "spf" / fp))
spf = np.array(spf)
...
for fp in os.listdir(DATA_DIR / "spf_surrogate" / "100000"):
    spf_surrogate.append(np.load(DATA_DIR / "spf_surrogate" / "100000" / fp))
...
for fp in os.listdir(DATA_DIR / "floodgate" / "10000"):
    floodgate.append(np.load(DATA_DIR / "floodgate" / "10000" / fp))
floodgate = np.array(floodgate)
floodgate_mean = np.mean(floodgate, axis=0)
floodgate_cov = np.mean((floodgate[:,:,:,0] < gt) * (floodgate[:,:,:,1] > gt), axis=0)
...
for fp in os.listdir(DATA_DIR / "spf_surrogate" / "10000"):
    spf_surrogate.append(np.load(DATA_DIR / "spf_surrogate" / "10000" / fp))
spf_surrogate = np.array(spf_surrogate)
spf_surrogate_mean = np.mean(spf_surrogate, axis=0)
spf_surrogate_cov = np.mean((spf_surrogate[:,:,:,0] < gt) * (spf_surrogate[:,:,:,1] > gt), axis=0)
...
for i in range(d):
    ...
    ax[0,i].axhline(y=gt[i], color="red", ls=":", lw=lw, label="Target", zorder=5)
    ax[0,i].axhline(y=surrogate_10k_gt[i], color="orange", ls=":", lw=lw, label="Low-quality $f$ Asymptotic")
    ax[0,i].axhline(y=surrogate_100k_gt[i], color="orange", ls="-.", lw=lw, label="High-quality $f$ Asymptotic")
    ax[1,i].axhline(y=0.95, color="red", ls=":", lw=lw)
    ax[1,i].set_ylim((0.5,1.05))
    if i > 0:
        ax[0,i].set_yticks([])
        ax[1,i].set_yticks([])
    ax[0,i].set_xticks([])
...
ax[0,2].legend(loc="lower center", bbox_to_anchor=(0.5, -.6), ncol=4, fancybox=True)
...
for fp in os.listdir(DATA_DIR / "panin" / "100000"):
    panin.append(np.load(DATA_DIR / "panin" / "100000" / fp))
...
for fp in os.listdir(DATA_DIR / "panin" / "10000"):
    panin.append(np.load(DATA_DIR / "panin" / "10000" / fp))
panin = np.array(panin)
panin_mean = np.mean(panin[:,:,:,1] - panin[:,:,:,0], axis=0)
panin_cov = np.mean((panin[:,:,:,0] < gt) * (panin[:,:,:,1] > gt), axis=0)
...
ax[1,0].set(ylabel="Coverage")
ax[1,2].set_xlabel("Computational Budget $N$", fontsize=HUGE_SIZE)
ax[0,2].legend(loc="lower center", bbox_to_anchor=(0.5, -1.75), ncol=3, fancybox=True)
ax[1,2].xaxis.labelpad = 15
